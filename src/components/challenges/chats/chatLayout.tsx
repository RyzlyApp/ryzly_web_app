...
export default function ChatLayout({ item }: { item: IChallenge }) {
  ...
  useEffect(() => {
    ...
    const handler = (data: any) => {
      console.log("THIS IS THE CHATS from the socket", data);
      const clone = uniqBy([...dataChat, data], "_id");
      setD(clone);
    };
        ...
        <div className=" w-full flex flex-col h-[75vh] ">
          ...
          <div className=" w-full flex flex-col-reverse h-full overflow-y-auto gap-2 py-1 ">
            <LoadingLayout loading={loading}>
              {[...dataChat, ...d]?.map((item, index) => {
                return (
                  <ChatCard
                    key={index}
                    item={item}
                    self={item?.senderId === user?._id}
                    previousDate={dataChat[index - 1]?.createdAt}
                  />
                );
              })}
            </LoadingLayout>
