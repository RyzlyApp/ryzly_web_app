...
export default function Overview({ item }: { item: IChallenge }) {
    const { data, isLoading } = useFetchData<IOverview>({
        endpoint: `/overview/${item?.overview}`,
        name: "overview",
    });

    const { formik, overviewMutate, tab, setTab, indexData, setIndexData } =
        useOverview();
    ...
    const deleteHandler = (
        index: number,
        name: "includes" | "requirements" | "whoIs" | "rules" | "outcomes",
    ) => {
            ...
            <div className=" w-full justify-between gap-4 items-center flex ">
                ...
                {isCoach && (
                    <div className=" flex gap-3 ">
                        <button
                            type="button"
                            onClick={() => clickHandler(name, index)}
                            className=" w-8 h-8 rounded-full flex text-neonblue-600 justify-center items-center bg-neonblue-50 "
                        >
                            <RiEditLine
                        ...
                        <button
                            type="button"
                            onClick={() => deleteHandler(index, name)}
                            className=" w-8 h-8 rounded-full flex text-neonblue-600 justify-center items-center bg-neonblue-50 "
                        >
            ...
            <FormikProvider value={formik}>
                <div className=" w-full flex flex-col p-4 gap-4 ">
                    ...
                    <div className=" w-full flex flex-col py-2 gap-2 ">
                        ...
                        {tab === "rules" && (
                            <OverviewForm
                                isLoading={overviewMutate?.isPending}
                                name={"rules"}
                                formik={formik}
                                setIsOpen={setTab}
                                index={
                        ...
                        {tab === "requirements" && (
                            <OverviewForm
                                ...
                                index={
                                    indexData > -1
                                        ? indexData
                                        : Number(data?.requirements?.length)
                                }
                            />
                        )}
                    </div>
