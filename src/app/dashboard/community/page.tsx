...
import { Skeleton, Button, Tabs, Tab } from "@heroui/react";
...
// px from the top of a feed where the composer is considered "at top" and shown
const COMPOSER_TOP_OFFSET = 40;
...
const CommunityPage = () => {
    ...
    const showAuthToast = useCallback(() => {
        addToast({
            ...
            description:
                "Create a free account or sign in to like, comment, and post.",
            color: "warning",
            timeout: 5000,
            endContent: (
    ...
    // Shared handlers passed to both carousel instances
    ...
    useEffect(() => {
        if (!isLoadingMessages && messages.length > 0 && !hasScrolled.current) {
            ...
            requestAnimationFrame(() => {
                scrollRefLatest.current?.scrollTo({
                    top: scrollRefLatest.current.scrollHeight,
                });
                scrollRefMy.current?.scrollTo({
                    top: scrollRefMy.current.scrollHeight,
                });
            });
        }
    }, [isLoadingMessages, messages.length]);

    // ── Composer visibility (only shows once a feed is scrolled to the top) ──
    const [isComposerVisible, setIsComposerVisible] = useState(true);
    const [activeFeedTab, setActiveFeedTab] = useState<"my" | "latest">(
        "latest",
    );
    ...
    const handleScroll =
        ...
        (e: React.UIEvent<HTMLDivElement>) => {
            ...
            setIsComposerVisible(el.scrollTop <= COMPOSER_TOP_OFFSET);
        };
    ...
    const handleShowComposer = () => {
        // const activeRef = activeFeedTab === "my" ? scrollRefMy : scrollRefLatest;
        // scrollToTop(activeRef)();
        // scrollToTop fires the scroll handler asynchronously as it animates,
        // so flip this immediately for instant feedback on press
        setIsOpen(true);
    };
    ...
    const tabsClassNames = {
        ...
        tabList:
            "flex items-start gap-6 w-full p-0 relative bg-white rounded-none border-b border-gray-100",
        tabContent: "group-data-[selected=true]:text-[#000]",
        panel: "p-0 flex-1 min-h-0 h-full overflow-y-hidden",
    };
            ...
            <div className="flex-1 flex min-h-0 overflow-hidden">
                ...
                <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                    <div className="flex-1 flex flex-col overflow-y-auto scrollbar-hide py-4 px-0 lg:px-4 gap-3 min-h-0">
                        {/* Composer — only visible when the active feed is scrolled to the top */}
                        {isComposerVisible && (
                        ...
                        <div className="flex flex-col min-h-[80vh] lg:min-h-0 lg:flex-1 overflow-hidden scrollbar-hide">
                            {isLoadingMessages ? (
                                ...
                                <div className="bg-white rounded-2xl flex flex-col h-full min-h-0 overflow-hidden border scrollbar-hide border-gray-100 relative">
                                    <Tabs
                                        aria-label="Feed tabs"
                                        variant="underlined"
                                        classNames={tabsClassNames}
                                        selectedKey={activeFeedTab}
                                    ...
                                    {/* Bring the composer back by jumping the active feed to the top */}
                                    {!isComposerVisible && (
                                        <Button
                                            isIconOnly
                                            radius="full"
                                            onPress={handleShowComposer}
                                            className="absolute bottom-4 right-4 z-20 bg-[#5160E7] text-white shadow-lg"
                        ...
                        {/* Mobile: activity log — below feed */}
                        {isAuth && (
