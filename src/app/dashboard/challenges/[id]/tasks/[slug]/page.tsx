...
export default function TaskPage() {
    ...
    const { data, isLoading } = useFetchData<ITaskDetail>({ endpoint: `/task/${id}` })
        ...
        <Loader loading={isLoading} >  
            <div className=" w-full flex lg:flex-row flex-col h-full gap-4 " >
                <div className=" w-full lg:w-fit h-fit bg-white p-4 rounded-2xl " >
                    <TaskDetails isCoach={user?._id === data?.creator?._id} item={data as ITaskDetail} />
                </div> 
                <SubmissionDetails isCoach={user?._id === data?.creator?._id} item={data as ITaskDetail} />
            </div>
        </Loader>
    )
}